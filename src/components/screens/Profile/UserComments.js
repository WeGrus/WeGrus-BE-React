...
function UserComments(props) {
  ...
  const [target, setTarget] = React.useState(
    location.state ? location.state.category : "INFO"
  ); // 게시판중 사이드바와 분류를 나눔. 즉 subCategory
  ...
  const handleSearchFunction = (data, currentBoardType, currentType, page) => {
    if (data.option === "제목 + 내용") {
      axios
        .get(
          `/search/all/${currentBoardType}?keyword=${data.keyword}&page=${
            page - 1
      ...
      axios
        .get(
          `/search/title/${currentBoardType}?keyword=${data.keyword}&page=${
            page - 1
          }&pageSize=19&type=${"LASTEST"}`,
          {
            headers: { Authorization: `Bearer ${props.userReducer.token}` },
          }
        )
      ...
      axios
        .get(
          `/search/writer/${currentBoardType}?keyword=${data.keyword}&page=${
            page - 1
          }&pageSize=19&type=${"LASTEST"}`,
          {
            headers: { Authorization: `Bearer ${props.userReducer.token}` },
  ...
  React.useEffect(() => {
    ...
    if (subCategory !== undefined) {
      const boardType = subCategory.find(
  ...
  React.useEffect(() => {
    ...
    if (subCategory !== undefined) {
      ...
      if (page === 1) {
        axios
          .get(`/boards/${boardType}?page=${0}&pageSize=20&type=${"LASTEST"}`, {
            headers: { Authorization: `Bearer ${props.userReducer.token}` },
          })
          .catch(function (error) {
            console.log(error.toJSON());
    ...
    if (subCategory !== undefined) {
      if (page === 1) {
        if (isSearch === false) {
          ...
          axios
            .get(
              `/boards/${currentBoardType}?page=${0}&pageSize=19&type=${selected}`,
              {
                headers: { Authorization: `Bearer ${props.userReducer.token}` },
              }
            )
  ...
  React.useEffect(() => {
    if (subCategory !== undefined) {
      if (isSearch === false) {
        axios
          .get(
            `/boards/${currentBoardType}?page=${
              page - 1
            }&pageSize=20&type=${currentType}`,
            {
              headers: { Authorization: `Bearer ${props.userReducer.token}` },
            }
          )
