...
function Study(props) {
    ...
    const handleSearching = (data, e) => {
        // 사용자가 검색을 했을때
        console.log(data);
        let url = `/study/${param.boardId}/1/LASTEST/true?option=${data.option}&keyword=${data.keyword}`
    ...
    const handleSearchBarFilter = (e) => {
        ...
        if(param.isSearch === "false"){
            navigate(`/study/${param.boardId}/${page}/${type}/false`);
        }
        else if(param.isSearch === "true"){
            let url = `/study/${param.boardId}/${page}/${type}/true?option=${searchParams.get("option")}&keyword=${searchParams.get("keyword")}`
            url= url.replace(/\+/g,"%2B");
            navigate(url);
        }
    };
