...
function CommentSection(props) {
  ...
  const commentData = props.commentData.filter(
    (item) => item.replyParentId === -1
  );
  const reCommentData = props.commentData.filter(
    (item) => item.replyParentId !== -1
  );
  ...
  const checkBlank = (value) => {
    // 댓글을 입력할 때 공백만 입력하는지 확인하는 함수. ex) comment = "         " 이런 값일 때.
    let isBlank = true;
  ...
  const handleCommentDelete = (e) => {
    ...
    if (value) {
      axios
        .delete(`/comments?replyId=${replyId}`, {})
        .catch(function (error) {
          console.log(error.toJSON());
        })
        .then(function (res) {
  ...
  const handleCommentRecommandCancel = (e) => {
    ...
    axios
      ...
      .then(function (res) {
        ...
        console.log(
          (e.target.parentNode.parentNode.childNodes[0].childNodes[1].childNodes[1].childNodes[1].childNodes[1].textContent =
            changeNum)
        );
        triggerSwitch();
      });
  ...
  const handleCommentRecommand = (e) => {
    // 댓글 추천 함수
    const info = e.target.parentNode.parentNode.dataset.id;
    const commentId = Number(info);
    ...
    axios
      .post(`/comments/like?replyId=${commentId}`, {}, {})
      .catch(function (error) {
        console.log(error);
      })
  ...
  const handleReCommentWirte = (e) => {
    // 댓글에서 답글 버튼을 눌렀을 때 동작하는 함수
    let index = e.target.parentNode.parentNode.dataset.id;
    if (index === commentIndex) index = -1;
    setCommentIndex(index);
    //console.log(e);
  };

  const handleReCommentSubmit = (e) => {
    // 대댓글을 전송하는 함수
    const parentId = Number(reCommentEl.current.dataset.id);

    const recomment = reCommentEl.current.value;

    if (checkBlank(recomment)) {
      // 댓글을 달지 않고 버튼을 클릭했을 때.
      reCommentEl.current.placeholder =
        "  내용이 없는 댓글은 등록하실 수 없습니다.";
    } else {
      axios
        .post(
          `/comments`,
          ...
          <>
            <ReComment key={reComment.replyId} data-id={`${reComment.replyId}`}>
              ...
              {reComment.memberId === props.userReducer.id ? (
                <BtnBar data-index={reComment.replyId}>
                  {reComment.userReplyLiked === false ? (
                    <CommentSpan onClick={handleCommentRecommand}>
                      추천
                    </CommentSpan>
                  ) : (
                    <CommentSpan onClick={handleCommentRecommandCancel}>
                  ...
                  |{" "}
                  <CommentSpan onClick={handleReCommentDelete}>
                    삭제
                  </CommentSpan>
                </BtnBar>
              ) : (
      ...
      {comment.replyId === commentIndex ? (
        <CommentWriteSection>
          <CommentWrite
            ref={reCommentEl}
            onChange={(e) => {}}
            placeholder={"댓글 작성시 네티켓을 지켜주세요."}
            required
            data-id={comment.replyId}
          ></CommentWrite>
          <CommentSubmit onClick={handleReCommentSubmit}>
            답글 등록
          </CommentSubmit>
        </CommentWriteSection>
      ) : null}
  ...
  return (
    <>
      <Comments id="commentTag">{printComment}</Comments>
